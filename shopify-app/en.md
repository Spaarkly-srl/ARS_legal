...
This policy is specific to the Shopify App and is limited to the data processed in connection with Shopify (merchant, store, product, configuration and attribution data). The processing performed by ARShades end-user services — such as Virtual Try-On, 3D/AR Viewer and AR PD Meter — that may be launched through the App is not covered by this policy and is governed by the **ARShades core Privacy Policy** (version 5.3), available from Spaarkly and displayed to shoppers within the ARShades experiences.
